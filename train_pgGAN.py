        ...
        for step in range(2, self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK, self.loader.batchsize)):
                ...
                if self.use_tb:
                    with torch.no_grad():
                        x_test = self.G(self.z_test)
                    self.writer.add_scalar('data/loss_g', loss_g.item(), self.globalIter)
                    self.writer.add_scalar('data/loss_d', loss_d.item(), self.globalIter)
                    self.writer.add_scalar('tick/lr', self.lr, self.globalIter)
                    self.writer.add_scalar('tick/cur_resl', int(pow(2,floor(self.resl))), self.globalIter)
                    '''IMAGE GRID
