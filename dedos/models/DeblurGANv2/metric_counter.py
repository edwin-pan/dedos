...
class MetricCounter:
    ...
    def write_to_dict(self, epoch, save_path, identifier, val=False):
        for key, value in self.metrics.items():
            self.metric_dict[key].append(np.mean(value))
        
        if val:
            np.save(os.path.join(save_path, f"{identifier}/dedos_vals/dedos_metrics/metrics_val.npy"), self.metric_dict)
            np.save(os.path.join(save_path, f"{identifier}/dedos_vals/dedos_metrics/image_val{epoch}.npy"), self.images)
        else:
