...
class MetricCounter:
    ...
    def write_to_dict(self, epoch, identifier, val=False):
        for key, value in self.metrics.items():
            self.metric_dict[key].append(np.mean(value))
        
        if val:
            np.save(f"/home/clairezhangbin/cs236/{identifier}/dedos_vals/dedos_metrics/metrics_val.npy", self.metric_dict)
            np.save(f"/home/clairezhangbin/cs236/{identifier}/dedos_vals/dedos_metrics/image_val{epoch}.npy", self.images)
        else:
            np.save(f"/home/clairezhangbin/cs236/{identifier}/dedos_vals/dedos_metrics/metrics_train.npy", self.metric_dict)
            np.save(f"/home/clairezhangbin/cs236/{identifier}/dedos_vals/dedos_metrics/image_train{epoch}.npy", self.images)
